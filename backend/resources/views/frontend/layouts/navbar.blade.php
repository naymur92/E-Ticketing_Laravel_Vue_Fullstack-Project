<nav class="navbar sticky-top navbar-expand-lg navbar-light p-0" style="background-color: #e3f2fd">
  <div class="container">
    <a class="navbar-brand" href="/">E-Ticket</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent"
      aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link" href="/">Home</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/login">Login</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/register">Register</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/verify-ticket">Verify Ticket</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/train-information">Train Information</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/contact-us">Contact Us</a>
        </li>
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
            data-bs-toggle="dropdown" aria-expanded="false">
            <span>Naymur Rahman</span>
          </a>
          <ul class="dropdown-menu" aria-labelledby="navbarDropdown" style="background-color: #e3f2fd">
            <li>
              <span class="dropdown-item"><strong>Admin</strong></span>
            </li>
            <li>
              <span class="dropdown-item"><strong>Naymur Rahman</strong></span>
            </li>
            <li>
              <span class="dropdown-item">
                <i class="fa fa-envelope"></i>
                <span class="mx-2">naymurrahman@yahoo.com</span>
              </span>
            </li>
            <li>
              <span class="dropdown-item">
                <i class="fa fa-phone"></i>
                <span class="mx-2">01737036324</span>
              </span>
            </li>
            <li>
              <hr class="dropdown-divider" />
            </li>
            <li>
              <a class="dropdown-item" href="#">
                <i class="fa-solid fa-gauge-high"></i>
                <span class="mx-2">Dashboard</span>
              </a>
            </li>
            <li>
              <a class="dropdown-item" href="#">
                <i class="fa fa-user"></i>
                <span class="mx-2">Profile</span>
              </a>
            </li>
            <li>
              <a class="dropdown-item" href="#">
                <i class="fa fa-briefcase"></i>
                <span class="mx-2">Purchase History</span>
              </a>
            </li>
            <li>
              <a class="dropdown-item" href="#">
                <i class="fa fa-shield-halved"></i>
                <span class="mx-2">Update Password</span>
              </a>
            </li>
            <li>
              <a class="dropdown-item" href="#">
                <i class="fa fa-sign-out"></i>
                <span class="mx-2">Logout</span>
              </a>
            </li>
          </ul>
        </li>
      </ul>
    </div>
  </div>
</nav>
